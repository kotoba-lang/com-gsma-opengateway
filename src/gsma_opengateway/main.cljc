(ns gsma-opengateway.main
  "Kotodama WASM entrypoint for the GSMA OpenGateway clean-room actor (L4) — Clojure port.
  ...
  facts (`gsma_opengateway.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gsma_opengateway")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"gsma_opengateway.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gsma_opengateway-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gsma_opengateway-compat/ready)
