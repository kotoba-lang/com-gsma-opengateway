(ns gsma-opengateway.main-test
  "Contract + behavioral test for the gsma-opengateway L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "gsma_opengateway-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'gsma-opengateway.main-test)]
